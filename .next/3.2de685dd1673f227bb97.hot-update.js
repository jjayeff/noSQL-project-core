...
var Navbar = function (_React$Component) {
  ...
  _createClass(Navbar, [{
    ...
    value: function hendleClick(group) {
      if (group && group.name != 'ข้อมูลหนังทั้งหมด' && group.name != 'เพิ่มหนัง' && group.name != 'ข้อมูลนักแสดงทั้งหมด') {
        this.props.sreachGroupMovie(group.name);
        __WEBPACK_IMPORTED_MODULE_2_next_router___default.a.push({
          pathname: group.link,
          query: { group: group.name }
        });
      } else if (group.name === 'ข้อมูลหนังทั้งหมด' || group.name === 'เพิ่มหนัง' || group.name === 'ข้อมูลนักแสดงทั้งหมด') {
        if (group.name === 'ข้อมูลหนังทั้งหมด') {
          this.props.fetchMovie();
          __WEBPACK_IMPORTED_MODULE_2_next_router___default.a.push({
            pathname: group.link,
            query: { sreach: 'movie' }
          });
        } else if (group.name === 'ข้อมูลนักแสดงทั้งหมด') {
          this.props.fetchMovie();
          __WEBPACK_IMPORTED_MODULE_2_next_router___default.a.push({
            pathname: group.link,
            query: { sreach: 'actor' }
...
//# sourceMappingURL=3.2de685dd1673f227bb97.hot-update.js.map